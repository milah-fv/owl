...
@section('subtitulo2', 'Lista de Exportadores')
      ...
      <div class="card ">
        <div class="card-header pb-0">
          <div class="text-center col-md-4 col-sm-12">
            ...
            href="{{ url('exportador/create') }}">Agregar Exportador </a>
          </div>
        </div><br>
          ...
          <div class="table-responsive p-2">
            <table class="table align-items-center justify-content-center mb-0" id="exportadores">
              ...
              <tbody>
                ...
                <tr >
                  <td>{{$exportador->updated_at}}</td>
                  <td  class="align-middle text-center">
                    <p class="text-xs font-weight-bold mb-0">{{$exportador->id}}</p>
                  </td>
                  <td >
                    <div class="my-auto">
                      <h6 class="mb-0 text-xs font-weight-bold" >{{$exportador->nombre}}</h6>
                    </div>
                  </td>
                  <td >
                      <span class="text-xs font-weight-bold mb-0">{{$exportador->rfc}}</span>
                  </td>
                  <td>
                    <span class="text-xs font-weight-bold">{{$exportador->domicilio}}</span>
                  </td>
                  <td>
                    <span class="text-xs font-weight-bold">{{$exportador->contacto}}</span>
                  </td>
                  <td>
                    <span class="text-xs font-weight-bold">{{$exportador->telefono}}</span>
                  </td>
                  <td class="align-middle text-center">
                    <form action="{{ url('/exportador/'.$exportador->id) }}" method="post" class="formEliminar">
                      @csrf
                      {{method_field('DELETE')}}

                      <!-- <a class="btn btn-link text-dark px-3 mb-0" href="javascript:;"><i class="fas fa-eye text-dark me-2" aria-hidden="true"></i>Ver</a> -->
                      <a title="Editar" style="padding:0px;" class="btn btn-link text-dark px-3 mb-0" href="{{ url('/exportador/'.$exportador->id.'/edit') }}"  ><i class="fas fa-pencil-alt text-dark me-2" aria-hidden="true"></i></a>

                      <input  type="hidden" name="eliminarValor" value="{{ $exportador->id }}">
